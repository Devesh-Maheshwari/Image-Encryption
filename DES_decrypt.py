...
#print(np.amin(img1),np.amax(img1))

img2 = np.genfromtxt('test.csv',delimiter=',')
img2 = np.reshape(img2,(img1.shape[0],img1.shape[1],img1.shape[2]))
img= img2.flatten()
img= np.array(img).astype(int)

dec_img = np.zeros(img.shape[0])

key='1111011011010110100100001100101001101100110101010001001100000010'
nor=16
...
for i in range(0,img.shape[0],8):
    ...
    for k in range(64):
        ...
        cipher_text1 += cipher_text[init_per[k]-1]
    cipher_text= cipher_text1
        
    left_cipher = cipher_text[0:32]
    right_cipher = cipher_text[32:]
    #-------------------Generate key for rounds-----------------------------------
    left_key = new_key[0:28]
    right_key = new_key[28:]
    for k in range(15,-1,-1):
        shifts = int(shift_per_round[k])
        new_key1 = left_key + right_key
        com_key = ''
        for l in range(compress_key.shape[0]):
            com_key += new_key1[compress_key[l]-1]
        left_key_expansion = ''
        for l in range(right_side_expansion.shape[0]):
            left_key_expansion += left_cipher[right_side_expansion[l]-1]
            
        var = int(com_key,2)^int(left_key_expansion,2)
        var = "{:048b}".format(var)
        s_box = ''
        for l1 in range(8):
            l = 6*l1
        ...
        num = int(s_box,2)^int(right_cipher,2)
        right_cipher = left_cipher
        left_cipher = "{:032b}".format(num)
        right_key = right_key[shifts:] + right_key[0:shifts]
        left_key = left_key[28-shifts:] + left_key[0:28-shifts]
            
    plain_text = left_cipher + right_cipher
    plain_text1=''
    for k in range(64):
        plain_text1 += plain_text[rev_per[k]-1]
    plain_text = plain_text1
        
    for k in range(8):
        num1 = plain_text[8*k:8*k+8]
        num1 = int(num1,2)
        dec_img[i+k] = num1
dec_img = np.reshape(dec_img,(img1.shape[0],img1.shape[1],img1.shape[2]))
#print(dec_img.shape)
#print(img1)
#print(img1.shape)

#print("PSNR=",psnr)
cv2.imwrite("lena_dec.jpg",dec_img) 
#dec_img = cv2.imread("lena_dec.jpg")
mse = MSE_coloured(dec_img,img1)
psnr = PSNR_coloured(dec_img,img1)
sim=ssim(dec_img,img1,multichannel=True)
print("mse=",mse)
print("psnr=",psnr)
...
plt.savefig("xraykneepsowcnga.jpg_dec(r).png")
plt.hist(dec_img[1])
plt.savefig("xraykneepsowcnga.jpg_dec(g).png")
plt.hist(dec_img[2])
plt.savefig("xraykneepsowcnga.jpg_dec(b).png")


with open(r'results_dec.csv','a') as f:
    writer=csv.writer(f)
    writer.writerow(ans)
